...
#' @import tibble
#' @import stringr
#' @param gene_list is a named vector of scores where names must be ENTREZID and scores are a preferred number
...
#' @param onlypos a logical flag which drops any ES values returned as negative and thus correcting less harshly. Defaults to FALSE. We advise to set onlypos = TRUE when performing GSMGSEA on NMF vectors. 
#' @return nothing
#' @examples 
#' res <- GeneSetMatch::GSMGSEA(gene_list = gene_list, theGoFile = i, pval = 0.05, onlypos = FALSE)

GSMGSEA = function(gene_list, theGoFile, pval, verbose = TRUE, onlypos = FALSE) {
  set.seed(54321)
  library(dplyr)
  library(gage)
  library(fgsea)
