...
GSM.Heatmaps <-
  function(gsea_results,
           ...
           pdftofile = TRUE) {
    my_palette <-
      colorRampPalette(c("blue", "lightgrey", "red"))(n = 299)
    plots <- list()
    filelist <- list()
        ...
        if (length(theseResults$leadingEdge) > 1) {
          for (j in 1:length(theseResults$leadingEdge)) {
            ...
            if (length(theseLeadingGenes) > 0) {
              ...
              firstIndexWeWant <-
                match(as.character(theseLeadingGenes),
              ...
              theMatrix[j, firstIndexWeWant] <-
                thisAnalysis$orig_matrix$log2FoldChange[secondIndexWeWant]
            }
          }
          ...
          if (ncol(theMatrix) > 80) {
            ...
            keepers = unique(c(which(
              colnames(theMatrix) %in% names(overall)
            ), movers)) # combines top movers from each set ('movers') with consensus genes ('overall'), unique() removes overlap
          ...
          if (verbose) {
            print(paste0(i, "_", substr(names(
              thisAnalysis
            )[k],
            5, stop = 999), ".pdf"))
          }
          filelist[[i]][[k]] <-
            paste0(i, "_", substr(names(thisAnalysis)[k],
                                  5, stop = 999), ".pdf")
          
          if (pdftofile == TRUE) {
            pdf(
              paste0(i, "_", substr(names(
                thisAnalysis
              )[k],
              5, stop = 999), ".pdf"),
