...
#' GSM.Multilevel.Mouse.Heatmaps(gsea_results)
...
###
GSM.Multilevel.Mouse.Heatmaps <- function(gsea_results, verbose = TRUE, pdftofile = TRUE, filePrefix){
  ...
  #}
    ...
    if (pdftofile == TRUE) {
      dev.off()
    } #wrap in if statement, declare false in rmd.
  }
