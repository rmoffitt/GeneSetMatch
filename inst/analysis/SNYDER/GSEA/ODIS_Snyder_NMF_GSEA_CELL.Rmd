...
```{r}
Snyder_NMF_CELL <- readRDS("./Snyder_NMF_CELL.rds")
head(Snyder_NMF_CELL)

nmf_norm_cell <- BBmisc::normalize(Snyder_NMF_CELL, method = "range", range = c(0, 1), margin = 1L, on.constant = "quiet")
head(nmf_norm_cell)
df1 <- nmf_norm_cell[,1]
head(df1)
...
Gsea on All three vectors of NMF_CELL
```{r}

snyder_nmfs_cell <- list()

for(Y in list(list("V1" = nmf_norm_cell[,1, drop = F]),
              list("V2" = nmf_norm_cell[,2, drop = F]),
              list("V3" = nmf_norm_cell[,3, drop = F])) #drop = f MEANS KEEP IT AS A MATRIX WITH COLLUMNS AND 
){
  X <- Y[[1]]
  ...
  for(i in list_of_gmt_files){
    ...
    snyder_nmfs_cell[[names(Y)]][[i]] <- res  ## find a way to reference "nmf_norm" and "nmf_norm_org"
  }
  #orig_matrix must be a data frame, added conversion function for ENT to SYM
  snyder_nmfs_cell[[names(Y)]]$orig_matrix <- as.data.frame(list(log2FoldChange = gene_list,
                                                            ENTREZID = row.names(X), 
                                                            SYMBOL = MouseENT2MouseSYM(row.names(X)))) 
  
}

summary(snyder_nmfs_cell)
summary(snyder_nmfs_cell$V2)
summary(snyder_nmfs_cell$V2$orig_matrix)
head(snyder_nmfs_cell$V3$`./myC1.gmt`$Results)
str(snyder_nmfs_cell$V2$orig_matrix$log2FoldChange)

saveRDS(snyder_nmfs_cell, "./Snyder_NMF_CELL_GSEA.rds")
