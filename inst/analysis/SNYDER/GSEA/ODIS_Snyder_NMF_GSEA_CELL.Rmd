...
nmf_pm <- PatternMarker(Snyder_NMF_CELL)
head(nmf_pm)
...
Q = list(V1 = nmf_norm_cell[,1, drop = F],
         V2 = nmf_norm_cell[,2, drop = F],
         V3 = nmf_norm_cell[,3, drop = F])
...
for(Y in c("V1","V2","V3") #drop = f MEANS KEEP IT AS A MATRIX WITH COLLUMNS AND
  ...
  ## Make vector sorted gene list of TFPM scores
    ...
    Z <- Q[[Y]]
    orig_value = Z[,1]
    orig_value = sort(orig_value, decreasing = TRUE)
  ...
  for(i in list_of_gmt_files){
    ...
    snyder_nmfs_cell[[Y]][[i]] <- res  ## find a way to reference "nmf_norm" and "nmf_norm_org"
  }
  #orig_matrix must be a data frame, added conversion function for ENT to SYM
  snyder_nmfs_cell[[Y]]$orig_matrix <- as.data.frame(list(log2FoldChange = orig_value,
