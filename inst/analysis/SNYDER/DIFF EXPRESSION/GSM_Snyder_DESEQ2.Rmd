...
---
#This markdown uses full_Snyder mouse data to perform a differential expression analysis. The tool used for this step is DeSeq2, and the protocol is performed on the entire dataset as well as just the organoid subset. Results are saved as .rds files, and used in the subsequent pathway enrichment analytical step.
...
DATA
```{r}
full_Snyder <- GeneSetMatch::full_Snyder # this doesn't load
...
Create FeatInfo so that we have all types of GeneIDs
```{r}
library(biomaRt)
