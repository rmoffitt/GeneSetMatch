...
for(Y in list(list("V1" = nmf_tfpm[,1, drop = F]),
              list("V2" = nmf_tfpm[,2, drop = F]),
              list("V3" = nmf_tfpm[,3, drop = F])) #drop = f MEANS KEEP IT AS A MATRIX WITH COLLUMNS AND 
)
  for(Q in list(list("V1" = nmf_norm[,1, drop = F]),
                list("V2" = nmf_norm[,2, drop = F]),
                list("V3" = nmf_norm[,3, drop = F]))){
    X <- Y[[1]]
    gene_list = X[,1]
    ...
    for(i in list_of_gmt_files){
      ...
      LLB_NMF_GSEA_TFPM[[names(Y)]][[i]] <- res  ## find a way to reference "nmf_norm" and "nmf_norm_org"
    }
    #orig_matrix must be a data frame
    LLB_NMF_GSEA_TFPM[[names(Y)]]$orig_matrix <- as.data.frame(list(log2FoldChange = orig_value,
...
head(LLB_NMF_GSEA_TFPM$V2$`./myC1.gmt`$Results)
str(LLB_NMF_GSEA_TFPM$V2$orig_matrix$log2FoldChange)

saveRDS(LLB_NMF_GSEA_TFPM, "./LLB_NMF_GSEA_TFPM.rds")
