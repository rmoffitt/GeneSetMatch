...
C8_df = msigdbr(species = "Rattus norvegicus", category = "H", subcategory = "")
...
nmf_norm <- normalize(LLB_NMF, method = "range", range = c(0, 1), margin = 1L, on.constant = "quiet")
head(nmf_norm)
df1 <- nmf_norm[,1]
head(df1)
...
summary(LLB_NMF_GSEA$V2)
summary(LLB_NMF_GSEA$V2$orig_matrix)
head(LLB_NMF_GSEA$V2$`./myC1.gmt`$Results)
str(LLB_NMF_GSEA$V2$orig_matrix$log2FoldChange)

saveRDS(LLB_NMF_GSEA, "./LLB_NMF_GSEA_V123.rds")
