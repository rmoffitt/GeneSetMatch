...
---
This Markdown demonstrates how to apply the necessary normalizations and tranformations to the output of preceding NMF analysis. Once the normailizations have been applied, the modified GSMGSEA pathway enrichment protocol is performed.  
...
LiverLungBrain.rds
```{r}
 LLB_NMF_new <- LiverLungBrain$trueW
 head(LLB_NMF_new)
 
 LLB_NMF_new <- BBmisc::normalize(LLB_NMF_new, method = "range", range = c(0, 1), margin = 1L, on.constant = "quiet")
 head(LLB_NMF_new)
 df1 <- LLB_NMF_new[,1]
 head(df1)
...
Apply PatternMarker Transformation LiverLungBrain.rds
```{r}
 LLB_NMF_new <- LiverLungBrain$trueW
 head(LLB_NMF_new)

LLB_NMF_new_pm <- transformedPM(LLB_NMF_new)
head(LLB_NMF_new_pm)

rank <- 3
LLB_NMF_new_pm <- (sqrt(rank-1)/2 - LLB_NMF_new_pm)^3
head(LLB_NMF_new_pm)
df2 <- LLB_NMF_new_pm[,1]
head(df2)
```
...
for(Y in c("V1","V2","V3") #drop = f MEANS KEEP IT AS A MATRIX WITH COLLUMNS AND
  ...
  ## Make vector sorted gene list of TFPM scores
    ...
    for(i in list_of_gmt_files){
      ...
      LLB_TFPM_JAN[[Y]][[i]] <- res  ## find a way to reference "nmf_norm" and "nmf_norm_org"
    }
    #orig_matrix must be a data frame
    LLB_TFPM_JAN[[Y]]$orig_matrix <- as.data.frame(list(log2FoldChange = orig_value,
...
summary(LLB_TFPM_JAN)
summary(LLB_TFPM_JAN$V1)
summary(LLB_TFPM_JAN$V1$orig_matrix)
head(LLB_TFPM_JAN$V3$`./myC1.gmt`$Results)
str(LLB_TFPM_JAN$V2$orig_matrix$log2FoldChange)

saveRDS(LLB_TFPM_JAN, "./LLB_TFPM_JAN.rds")
